...
use reth_client::{
    custom::{mainnet, CustomEvmConfig},
    io::ClientExecutorInput,
    state::EthereumState,
};

pub use crate::rpc::RpcDb;
...
impl<T: Transport + Clone, P: Provider<T, AnyNetwork> + Clone> HostExecutor<T, P> {
    ...
    pub async fn execute(&self, block_number: u64) -> Result<ClientExecutorInput, HostError> {
        ...
        header.requests_root = current_block
            .body
            .requests
            .as_ref()
        ...
        if constructed_header_hash != target_hash {
            return Err(HostError::HeaderMismatch(
