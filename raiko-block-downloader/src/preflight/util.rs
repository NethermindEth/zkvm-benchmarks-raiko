...
use crate::{provider::{db::ProviderDb, rpc::RpcBlockDataProvider, BlockDataProvider}, RaikoResult};
...
pub async fn prepare_taiko_chain_input(
    ...
    let anchor_tx = block
        .body
        .first()
        .context("No anchor tx in the block")?;
    ...
    let l1_state_block_hash = l1_state_header.hash.context("No L1 state block hash for the requested block")?;
    let l1_inclusion_block_hash = l1_inclusion_header.hash.context("No L1 inclusion block hash for the requested block")?;
    info!(
    ...
    let (tx_data, blob_commitment, blob_proof) = if block_proposed.blob_used() {
        ...
        // Get the blob hashes attached to the propose tx and make sure the expected blob hash is in there
            ...
            "Proposal blobs hash mismatch: {:?} not in {:?}", expected_blob_hash, blob_hashes
        );
        ...
        match fork {
            ...
            _ => {
                ...
                let proposeBlockCall { txList, .. } =
                    proposeBlockCall::abi_decode(&proposal_tx.input, false).context("Could not decode proposeBlockCall")?;
                (txList.to_vec(), None, None)
            }
        }
    };
...
pub async fn get_tx_data(
    ...
    let beacon_rpc_url: String = chain_spec.beacon_rpc.clone().context("Beacon RPC URL is required for Taiko chains")?;
    ...
    BDP: BlockDataProvider,
{
    ...
    let Some(block) = blocks.next() else {
        bail!("No block data for the requested block");
    };
    let Some(parent_block) = blocks.next() else {
        bail!("No parent block data for the requested block");
    };
    ...
    let block = Block::try_from(block.clone())
        .context("Failed converting to reth block")?;
...
pub fn block_time_to_block_slot(
    ...
    if genesis_time == 0 {
        bail!("genesis time is 0, please check chain spec")
    } else if block_time < genesis_time {
        bail!("provided block_time precedes genesis time")
    } else {
