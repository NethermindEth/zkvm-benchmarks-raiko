...
use crate::provider::{db::ProviderDb, BlockDataProvider};
...
pub async fn preflight<BDP: BlockDataProvider>(
    ...
    let taiko_guest_input = if taiko_chain_spec.is_taiko() {
        ...
        TaikoGuestInput::try_from(block.body.clone()).map_err(|e| anyhow!("{}", e.0))?
    };
    measurement.stop();

    let parent_header: reth_primitives::Header =
        parent_block.header.clone().try_into().context("Failed converting to reth header")?;
