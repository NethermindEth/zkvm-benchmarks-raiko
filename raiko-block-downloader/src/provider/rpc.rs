...
//     MerkleProof,
// };
use super::RaikoResult;
use super::BlockDataProvider;
...
impl RpcBlockDataProvider {
    pub fn new(url: &str, block_number: u64) -> RaikoResult<Self> {
        let url =
            reqwest::Url::parse(url).map_err(|_| anyhow!("Invalid RPC URL {url}"))?;
        Ok(Self {
    ...
    async fn get_blocks(&self, blocks_to_fetch: &[(u64, bool)]) -> RaikoResult<Vec<Block>> {
        ...
        for blocks_to_fetch in blocks_to_fetch.chunks(max_batch_size) {
            ...
            for (block_number, full) in blocks_to_fetch {
                requests.push(Box::pin(
                    batch
                        ...
                        .context("Failed adding eth_getBlockByNumber call to batch")?
                ));
            }
            ...
            for request in requests {
                blocks.push(
                    request.await.context("Error collecting request data")?,
                );
            }
    ...
    async fn get_accounts(&self, accounts: &[Address]) -> RaikoResult<Vec<AccountInfo>> {
        ...
        for accounts in accounts.chunks(max_batch_size) {
            ...
            for address in accounts {
                nonce_requests.push(Box::pin(
                    batch
                        ...
                        .context("Failed adding eth_getTransactionCount call to batch")?,
                ));
                balance_requests.push(Box::pin(
                    batch
                        ...
                        .context("Failed adding eth_getBalance call to batch")?
                ));
                code_requests.push(Box::pin(
                    batch
                        ...
                        .context("Failed adding eth_getCode call to batch".to_owned())?
                ));
            }
            ...
            batch
                .send()
                .await
                .context("Error sending batch request")?;
                ...
                .zip(code_requests.into_iter())
            {
                let (nonce, balance, code) = (
                    nonce_request.await.context("Failed to collect nonce request")?,
                    balance_request.await.context("Failed to collect balance request")?,
                    code_request.await.context("Failed to collect code request")?,
                );

                let nonce = nonce.try_into().context("Failed to convert nonce to u64")?;
            ...
            for (address, key) in accounts {
                requests.push(Box::pin(
                    batch
                        ...
                        .context("Failed adding eth_getStorageAt call to batch")?,
                ));
            }
        ...
        while !accounts.is_empty() {
                ...
                if let Some((address, keys)) = accounts.iter_mut().next() {
                    ...
                    requests.push(Box::pin(
                        batch
                            ...
                            .context("Failed adding eth_getProof call to batch")?,
                    ));
            ...
            for request in requests {
                let mut proof = request
                    .await
                    .context("Error collecting request data")?;
